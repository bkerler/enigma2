...
void eDVBServiceFCCPlay::serviceEvent(int event)
{
	...
	switch (event)
	{
		...
		case eDVBServicePMTHandler::eventChannelAllocated:
		{
			...
			int slotId = getFrontendInfo(iFrontendInformation_ENUMS::frontendNumber);
...
void eDVBServiceFCCPlay::updateFCCDecoder(bool sendSeekableStateChanged)
{
	...
	if (m_decode_demux)
	{
		if (m_is_primary)
		{
			...
			m_teletext_parser->connectNewStream(sigc::mem_fun(*this, &eDVBServiceFCCPlay::newSubtitleStream), m_new_subtitle_stream_connection);
